...
export default function MarketPrices() {
  ...
  useEffect(() => {
    // Update data when crop changes
    setData(mockData[selectedCrop]);
  }, [selectedCrop]);

  // Mock live update simulation for current crop
  useEffect(() => {
    const interval = setInterval(() => {
      setData((prevData) => {
        const last = prevData[prevData.length - 1];
        const fluctuation = (Math.random() - 0.4) * 0.5; // Slight upward bias
        const newPrice = Math.max(10, last.price + fluctuation);
        const newDate = new Date();
        const dateStr = `${newDate.getHours()}:${newDate.getMinutes().toString().padStart(2, '0')}`;
        
        return [
          ...prevData.slice(1), // Remove oldest
          { date: dateStr, price: parseFloat(newPrice.toFixed(1)) }
        ];
      });
    }, 10000); // Update every 10s for demo (spec is 30s)
