...
export default function Dashboard() {
  const [prices, setPrices] = useState<PriceItem[]>([
    { name: 'Rice (Jaya)', price: 48.4, change: 1.2, unit: 'kg' },
    { name: 'Coconut', price: 38.0, change: -0.5, unit: 'piece' },
    { name: 'Tapioca', price: 32.5, change: 0.8, unit: 'kg' },
    { name: 'Banana (Nendran)', price: 65.0, change: 2.5, unit: 'kg' },
  ]);

  // Mock live price updates (Socket.io simulation)
  useEffect(() => {
    const interval = setInterval(() => {
      setPrices((prevPrices) =>
        prevPrices.map((item) => {
          const fluctuation = (Math.random() - 0.5) * 1;
          const newPrice = Math.max(10, item.price + fluctuation);
          return {
            ...item,
            price: parseFloat(newPrice.toFixed(1)),
            change: parseFloat(fluctuation.toFixed(1)),
          };
        })
      );
    }, 5000); // Update every 5s for demo, though spec says 30s for chart
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Good Morning, Suresh</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 md:col-span-1">
          <div className="flex justify-between items-start mb-4">
            <div>
              <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Palakkad, Kerala</div>
            ...
            <div className="space-y-3">
              <div className="flex gap-3 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-100 dark:border-amber-900/40">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-300/80">Heavy rainfall isolated in Palakkad starting tomorrow. Secure your harvest.</p>
                </div>
              </div>
