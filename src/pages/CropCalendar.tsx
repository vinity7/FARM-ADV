...
export default function CropCalendar() {
  ...
  useEffect(() => {
    const fetchCalendar = async () => {
      try {
        const { data } = await getCalendar(selectedDistrict.toLowerCase());
        if (data.schedule) {
          const apiEvents = data.schedule.map((item: any, idx: number) => ({
            id: `api-${idx}`,
            title: `${item.crop} (${item.season || 'Tapping'}): ${item.sowing || item.tapping || 'Activity'}`,
            start: new Date(new Date().getFullYear(), new Date().getMonth(), 10 + (idx * 2)).toISOString().split('T')[0], // Distribute across current month
            backgroundColor: item.sowing ? '#10b981' : item.tapping ? '#3b82f6' : '#f59e0b',
            borderColor: item.sowing ? '#059669' : item.tapping ? '#2563eb' : '#d97706'
          }));
          setEvents(apiEvents);
        }
      } catch (err) {
        console.error('API Calendar failed, using local mock', err);
        setEvents(districtEvents[selectedDistrict] || []);
      }
    };
    fetchCalendar();
  }, [selectedDistrict]);
